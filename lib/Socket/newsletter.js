...
export const extractNewsletterMetadata = (node, isCreate) => {
    ...
    if (errorNode) {
        throw new Boom(`newsletter query failed: ${errorNode.attrs?.text || 'unknown error'} (code ${errorNode.attrs?.code || '?'})`, { statusCode: +(errorNode.attrs?.code || 400) });
    }
    const resultNode = getBinaryNodeChild(node, 'result');
    const result = resultNode?.content?.toString();
    if (!result) {
        throw new Boom('newsletter query returned no result content, check invite/jid validity or query_id', { statusCode: 400 });
    }
    let parsed;
    try {
        parsed = JSON.parse(result);
    } catch (e) {
        throw new Boom(`newsletter query returned invalid JSON: ${result.slice(0, 200)}`, { statusCode: 400 });
    }
    const pathKey = isCreate ? XWAPaths.xwa2_newsletter_create : XWAPaths.xwa2_newsletter_metadata;
    const metadataPath = parsed?.data?.[pathKey];
    if (!metadataPath) {
        throw new Boom(`newsletter metadata missing expected path "${pathKey}" in response: ${JSON.stringify(parsed).slice(0, 300)}`, { statusCode: 400 });
    }
    const metadata = {
        id: metadataPath.id,
